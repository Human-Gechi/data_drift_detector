...
def plot_piechart(records, selected_table, full_table_name):
    if selected_table:
        ...
        if dtype_counts:
            ...
            fig.update_layout(
                title=f"Data Type Distribution - {selected_table}",
                height=500,
                showlegend=True,
            )
...
def main():
    ...
    with col1:
        st.metric("Total Tables Monitored", len(schemas))
    ...
    plot_piechart(records, selected_table, full_table_name)
    plot_check_tables(records)
